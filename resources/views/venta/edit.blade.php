...
@push('estilos')
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .cart-item .text-right.w-24 {
                width: auto;
                text-align: right;
            }

            .cart-item .flex.items-center.space-x-3 {
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-xl shadow-md p-6">
                        <h2 class="text-xl font-semibold mb-4 flex items-center">
                            <i class="fas fa-search mr-2 text-green-600"></i> Búsqueda de Productos
                        </h2>
                        <div class="relative">
                            <input type="text" id="productSearchInput"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg"
                                placeholder="Busca productos por nombre o código..." autocomplete="off">
                            <div id="searchResults"
                                class="search-results absolute w-full bg-white border rounded-lg shadow-lg mt-1 hidden">
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                            ...
                            @foreach (['efectivo', 'tarjeta', 'transferencia', 'yape', 'plin'] as $metodo)
                                @php
                                    ...
                                    $colors = [
                                        ...
                                        'yape' => 'text-purple-700',
                                        'plin' => 'text-blue-500',
                                    ];
                                @endphp
                                <button class="payment-method-btn p-4 border rounded-lg text-left flex items-center"
                                    data-method="{{ $metodo }}">
                    ...
                    <div class="bg-white rounded-xl shadow-md p-6">
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-between text-lg">
                                <span>IGV (18%):</span><span id="igv">S/ 0.00</span>
                            </div>
                            <hr>
                            <div class="flex justify-between text-2xl font-bold total-display p-4 rounded-lg">
                                <span>TOTAL:</span><span id="total">S/ 0.00</span>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @foreach ($venta->detalles as $detalle)
                cart.push({
                    ...
                    stock: parseInt("{{ $detalle->producto->stock }}") + parseInt(
                        "{{ $detalle->cantidad }}") // Stock actual + lo que estaba en esta venta
                });
            @endforeach
        ...
        function setupEventListeners() {
            ...
            document.querySelectorAll('.payment-method-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    document.querySelectorAll('.payment-method-btn').forEach(b => b.classList.remove(
                        'active'));
            ...
            fetch(`/api/pos/buscar-productos?termino=${encodeURIComponent(query)}`)
                ...
                .then(data => {
                    ...
                    if (!data.success || data.productos.length === 0) {
                        searchResultsEl.innerHTML =
                            '<div class="p-3 text-center text-gray-500">No se encontraron productos.</div>';
                        return;
                    }
        ...
        function updateCartDisplay() {
            ...
            if (cart.length === 0) {
                cartItemsEl.innerHTML =
                    `<div class="text-center text-gray-500 py-8"><i class="fas fa-shopping-cart text-4xl mb-4"></i><p>Carrito vacío</p></div>`;
                return;
            }
